...
void print_version()
{
  std::cout << "IfcOpenShell " << IfcSchema::Identifier << " IfcConvert " << IFCOPENSHELL_VERSION
            << " (OCC " << OCC_VERSION_STRING_EXT << ")\n";
}
...
void print_usage(bool suggest_help = true)
{
  std::cout << "Usage: IfcConvert [options] <input.ifc> [<output>]\n"
            ...
            << "\n"
            << "If no output filename given, <input>." + DEFAULT_EXTENSION +
...
const std::string NAME_ARG = "Name", GUID_ARG = "GlobalId", DESC_ARG = "Description",
                  TAG_ARG = "Tag";
...
size_t read_filters_from_file(const std::string &, inclusion_filter &, inclusion_traverse_filter &,
                              exclusion_filter &, exclusion_traverse_filter &);
...
int main(int argc, char **argv)
{
  ...
  generic_options.add_options()("help,h", "display usage information")(
      "version", "display version information")("verbose,v", "more verbose log messages")(
      "quiet,q", "less status and progress output")("stderr-progress",
                                                    "output progress to stderr stream")(
      "yes,y", "answer 'yes' automatically to possible confirmation queries (e.g. overwriting an "
       ...
       "Accepted values 'utf8' (the default) and 'escape'.")
          ...
          ("bounds", po::value<std::string>(&bounds),
           ...
           "output will be scaled. Only used when converting to SVG.")(
              ...
              "use-element-hierarchy", "Order the elements using their IfcBuildingStorey parent. "
                                       "Applicable for DAE output.")(
              "center-model",
              "Centers the elements upon serialization by applying the center point of "
              "all placements as an offset. Applicable for OBJ and DAE output. Can take several "
              "minutes on large models.")("model-offset", po::value<std::string>(&offset_str),
                                          "Applies an arbitrary offset of form 'x;y;z' to all "
                                          "placements. Applicable for OBJ and DAE output.")(
              ...
              "building-local-placement", "Similar to --site-local-placement, but placing elements "
                                          "in locally in the parent IfcBuilding coord system")(
              "precision",
              po::value<short>(&precision)->default_value(SerializerSettings::DEFAULT_PRECISION),
              "Sets the precision to be used to format floating-point values, 15 by default. "
              "Use a negative value to use the system's default precision (should be 6 typically). "
              "Applicable for OBJ and DAE output. For DAE output, value >= 15 means that up to 16 "
              "decimals are used, "
  ...
  {
    std::cerr << "[Error] Invalid or unsupported output file '" << output_filename << "' given"
              << std::endl;
    print_usage();
    return EXIT_FAILURE;
  }
  ...
  {
    std::string answer;
    std::cout << "A file '" << output_filename << "' already exists. Overwrite the existing file?"
              << std::endl;
    std::cin >> answer;
  ...
  {
    size_t num_filters =
        read_filters_from_file(filter_filename, include_filter, include_traverse_filter,
                               exclude_filter, exclude_traverse_filter);
    if (num_filters)
    {
                     ...
                     bool mmap)
{
  ...
  if (no_progress)
  {
    Logger::SetOutput(nullptr, &log_stream);
  }
...
bool append_filter(const std::string &type, const std::vector<std::string> &values,
                   geom_filter &filter)
{
  geom_filter temp;
...
size_t read_filters_from_file(const std::string &filename, inclusion_filter &include_filter,
                              inclusion_traverse_filter &include_traverse_filter,
                              exclusion_filter &exclude_filter,
                              exclusion_traverse_filter &exclude_traverse_filter)
{
  ...
  for (std::string line; std::getline(filter_file, line); ++line_number)
  {
    ...
    // Support both "--include=arg GlobalId 1VQ5n5$RrEbPk8le4ZCI81" and "include arg GlobalId
    // 1VQ5n5$RrEbPk8le4ZCI81" and tolerate extraneous whitespace.
...
void parse_filter(geom_filter &filter, const std::vector<std::string> &values)
{
  ...
  filter.values.insert(values.begin() + (filter.type == geom_filter::ENTITY_ARG ? 2 : 1),
                       values.end());
}

void validate(boost::any &v, const std::vector<std::string> &values, inclusion_filter *, int)
{
  /// @todo For now only single --include, --include+, --exclude, or --exclude+ supported. Support
  /// having multiple.
...
void validate(boost::any &v, const std::vector<std::string> &values, inclusion_traverse_filter *,
              int)
{
  po::validators::check_first_occurrence(v);
...
void validate(boost::any &v, const std::vector<std::string> &values, exclusion_traverse_filter *,
              int)
{
  po::validators::check_first_occurrence(v);
...
std::vector<IfcGeom::filter_t> setup_filters(const std::vector<geom_filter> &filters,
                                             const std::string &output_extension)
{
  ...
  if (!layer_filter.values.empty())
  {
    filter_funcs.emplace_back(boost::ref(layer_filter));
  }
  if (!entity_filter.values.empty())
  {
  ...
  if (!guid_filter.values.empty())
  {
    filter_funcs.emplace_back(boost::ref(guid_filter));
  }
  if (!name_filter.values.empty())
  {
    filter_funcs.emplace_back(boost::ref(name_filter));
  }
  if (!desc_filter.values.empty())
  {
    filter_funcs.emplace_back(boost::ref(desc_filter));
  }
  if (!tag_filter.values.empty())
  {
    filter_funcs.emplace_back(boost::ref(tag_filter));
  }
