...
template <typename T, typename U> int convert_to_ifc(const T& t, U*& u, bool /*advanced*/)
{
...
template <> int convert_to_ifc(const TopoDS_Vertex& v, IfcSchema::IfcVertex*& vertex, bool advanced)
{
...
template <> int convert_to_ifc(const gp_Ax2& a, IfcSchema::IfcAxis2Placement3D*& ax, bool advanced)
{
  IfcSchema::IfcCartesianPoint* p;
  IfcSchema::IfcDirection *x, *z;
  if (!(convert_to_ifc(a.Location(), p, advanced) && convert_to_ifc(a.Direction(), z, advanced) &&
...
template <typename T, typename U> void opencascade_array_to_vector(T& t, std::vector<U>& u)
{
  u.reserve(t.Length());
  ...
  if (c->DynamicType() == STANDARD_TYPE(Geom_Line)) {
    ...
    if (rational) {
      curve = new IfcSchema::IfcRationalBSplineCurveWithKnots(
          ...
          IfcSchema::IfcBSplineCurveForm::IfcBSplineCurveForm_UNSPECIFIED, bspline->IsClosed() != 0,
          false, mults, knots, knot_spec, weights);
    } else {
      curve = new IfcSchema::IfcBSplineCurveWithKnots(
          bspline->Degree(), points,
          IfcSchema::IfcBSplineCurveForm::IfcBSplineCurveForm_UNSPECIFIED, bspline->IsClosed() != 0,
          false, mults, knots, knot_spec);
    }
...
int convert_to_ifc(const Handle_Geom_Surface& s, IfcSchema::IfcSurface*& surface, bool advanced)
{
  if (s->DynamicType() == STANDARD_TYPE(Geom_Plane)) {
    ...
    bool rational = false;
    for (std::vector<std::vector<double>>::const_iterator it = weights.begin(); it != weights.end();
         ++it) {
    ...
    if (rational) {
      surface = new IfcSchema::IfcRationalBSplineSurfaceWithKnots(
          ...
          bspline->IsUClosed() != 0, bspline->IsVClosed() != 0, false, umults, vmults, uknots,
          vknots, knot_spec_u, weights);
    } else {
...
template <> int convert_to_ifc(const TopoDS_Edge& e, IfcSchema::IfcCurve*& c, bool advanced)
{
  double a, b;
  ...
  c = new IfcSchema::IfcTrimmedCurve(
      base, trim1, trim2, true, IfcSchema::IfcTrimmingPreference::IfcTrimmingPreference_PARAMETER);

  return 1;
}

template <> int convert_to_ifc(const TopoDS_Edge& e, IfcSchema::IfcEdge*& edge, bool advanced)
{
  double a, b;
...
template <> int convert_to_ifc(const TopoDS_Wire& wire, IfcSchema::IfcLoop*& loop, bool advanced)
{
  bool polygonal = true;
  ...
  if (!polygonal && !advanced) {
    ...
    for (; exp.More(); exp.Next()) {
      ...
      // With advanced set to true convert_to_ifc(TopoDS_Edge&) will always create an
      // IfcOrientedEdge
...
template <> int convert_to_ifc(const TopoDS_Face& f, IfcSchema::IfcFace*& face, bool advanced)
{
...
template <typename U> int convert_to_ifc(const TopoDS_Shape& s, U*& item, bool advanced)
{
...
IfcSchema::IfcProductDefinitionShape* IfcGeom::serialise(const TopoDS_Shape& shape, bool advanced)
{
  ...
  if (items->size() > 0) {
    ...
    if (shells->size() > 0) {
      ...
      rep =
          new IfcSchema::IfcShapeRepresentation(0, std::string("Body"), std::string("Brep"), items);
    } else {
      ...
      if (face_count > 0) {
        items->push(shell);
        rep = new IfcSchema::IfcShapeRepresentation(0, std::string("Body"), std::string("Brep"),
                                                    items);
      } else {
        ...
        if (edges->size() == 0) {
          ...
          rep =
              new IfcSchema::IfcShapeRepresentation(0, std::string("Axis"), std::string("Curve2D"),
...
IfcSchema::IfcProductDefinitionShape* IfcGeom::tesselate(const TopoDS_Shape& shape,
                                                         double deflection)
{
  ...
  for (TopExp_Explorer exp(shape, TopAbs_FACE); exp.More(); exp.Next()) {
    ...
    if (!tri.IsNull()) {
      ...
      for (int i = 1; i <= triangles.Length(); ++i) {
        ...
        IfcSchema::IfcFaceOuterBound* bound =
            new IfcSchema::IfcFaceOuterBound(loop, face.Orientation() != TopAbs_REVERSED);
        IfcSchema::IfcFaceBound::list::ptr bounds(new IfcSchema::IfcFaceBound::list);
        bounds->push(bound);
