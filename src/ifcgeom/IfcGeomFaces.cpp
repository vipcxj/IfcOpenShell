...
bool IfcGeom::Kernel::convert(const IfcSchema::IfcFace *l, TopoDS_Shape &face)
{
  ...
  if (is_face_surface)
  {
    ...
    if (!convert_shape(fs->FaceSurface(), surface_shape))
      return false;
  ...
  // The builder is initialized on the heap because of the various different
  // moments of initialization depending on the configuration of surfaces and
  // boundaries.
  ...
  for (int process_interior = 0; process_interior <= 1; ++process_interior)
  {
    for (IfcSchema::IfcFaceBound::list::it it = bounds->begin(); it != bounds->end(); ++it)
    {
      ...
      if (loop->is(IfcSchema::Type::IfcPolyLoop) && processed == 0 &&
      face_surface.IsNull()) { IfcSchema::IfcPolyLoop* polyloop =
      (IfcSchema::IfcPolyLoop*) loop; IfcSchema::IfcCartesianPoint::list::ptr
      points = polyloop->Polygon();

              if (points->size() == 3) {
                      // Help Open Cascade by finding the plane more efficiently
                      IfcSchema::IfcCartesianPoint::list::it point_iterator =
      points->begin(); gp_Pnt a, b, c; convert(*point_iterator++, a);
...
bool IfcGeom::Kernel::convert(const IfcSchema::IfcZShapeProfileDef *l, TopoDS_Shape &face)
{
  ...
  double coords[16] = {-dx, -y, x,  -y, x,  -y + dy, dx,  -y + dy,
