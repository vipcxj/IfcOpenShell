...
{
  /// @todo public/private sections all over the place: move all public to the beginning of the
  /// class
...
public:
  ...
  /// Moves to the next shape representation, create its geometry, and returns the associated
  /// product. Use get() to retrieve the created geometry.
    ...
    if (next_shape_model)
    {
      ...
      else if (!settings.get(IteratorSettings::DISABLE_TRIANGULATION))
      {
        try
        {
          ...
          {
            next_triangulation = new TriangulationElement<P>(
...
private:
  ...
  // A container and iterator for IfcBuildingElements for the current IfcRepresentation referenced
  // by *representation_iterator
  ...
  BRepElement<P> *create_shape_model_for_next_entity()
  {
    ...
    for (;;)
    {
      ...
      std::cout << "count: "<<count<<" cbrep0: "<<count_brep0<<" cbrep1: " << count_brep1<<std::endl;
      return element;
    }
  }
