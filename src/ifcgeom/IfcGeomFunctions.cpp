...
#ifdef _MSC_VER
#pragma message(                                                                                   \
    "warning: You are linking against Open CASCADE version " OCC_VERSION_COMPLETE                  \
    ". Version 6.9.0 introduces various improvements with relation to boolean operations. You are advised to upgrade.")
#else
#warning                                                                                           \
    "You are linking against linking against an older version of Open CASCADE. Version 6.9.0 introduces various improvements with relation to boolean operations. You are advised to upgrade."
#endif
#endif
...
void bounding_box_overlap(double p, const TopoDS_Shape &a, const TopTools_ListOfShape &b,
...
bool IfcGeom::Kernel::create_solid_from_faces(const TopTools_ListOfShape &face_list,
                                              TopoDS_Shape &shape)
{
...
const TopoDS_Shape &IfcGeom::Kernel::ensure_fit_for_subtraction(const TopoDS_Shape &shape,
                                                                TopoDS_Shape &solid)
{
...
bool IfcGeom::Kernel::convert_openings(const IfcSchema::IfcProduct *entity,
                                       const IfcSchema::IfcRelVoidsElement::list::ptr &openings,
                                       const IfcGeom::IfcRepresentationShapeItems &entity_shapes,
                                       const gp_Trsf &entity_trsf,
                                       IfcGeom::IfcRepresentationShapeItems &cut_shapes)
{
  ...
  for (IfcGeom::IfcRepresentationShapeItems::const_iterator it3 = entity_shapes.begin();
       it3 != entity_shapes.end(); ++it3)
  {
    TopoDS_Shape entity_shape_solid;
    const TopoDS_Shape &entity_shape_unlocated =
        ensure_fit_for_subtraction(it3->Shape(), entity_shape_solid);
    const gp_GTrsf &entity_shape_gtrsf = it3->Placement();
    if (entity_shape_gtrsf.Form() == gp_Other)
    {
      Logger::Message(Logger::LOG_WARNING,
                      "Applying non uniform transformation to:", entity->entity);
    }
    ...
    for (IfcGeom::IfcRepresentationShapeItems::const_iterator it4 = opening_shapes.begin();
         it4 != opening_shapes.end(); ++it4)
    {
      TopoDS_Shape opening_shape_solid;
      const TopoDS_Shape &opening_shape_unlocated =
          ensure_fit_for_subtraction(it4->Shape(), opening_shape_solid);
      const gp_GTrsf &opening_shape_gtrsf = it4->Placement();
      if (opening_shape_gtrsf.Form() == gp_Other)
      {
        Logger::Message(Logger::LOG_WARNING,
                        "Applying non uniform transformation to opening of:", entity->entity);
      }
      TopoDS_Shape opening_shape =
          apply_transformation(opening_shape_unlocated, opening_shape_gtrsf);
      ...
      else
      {
        ...
        if (brep_cut.IsDone())
        {
          ...
          {
            Logger::Message(Logger::LOG_WARNING,
                            "Shape healing failed on opening subtraction result", entity->entity);
          }
          ...
          if (is_valid)
          {
            ...
            if (Logger::LOG_WARNING >= Logger::Verbosity())
            {
              ...
              if (ALMOST_THE_SAME(original_shape_volume, volume_after_subtraction))
                Logger::Message(Logger::LOG_WARNING,
                                "Subtraction yields unchanged volume:", entity->entity);
            }
          }
          else
          {
...
#if OCC_VERSION_HEX < 0x60900
bool IfcGeom::Kernel::convert_openings_fast(
    const IfcSchema::IfcProduct *entity, const IfcSchema::IfcRelVoidsElement::list::ptr &openings,
    const IfcGeom::IfcRepresentationShapeItems &entity_shapes, const gp_Trsf &entity_trsf,
    IfcGeom::IfcRepresentationShapeItems &cut_shapes)
{
       ...
       it3 != entity_shapes.end(); ++it3)
  {
    ...
    if (!is_valid)
    {
      ...
      Logger::Message(Logger::LOG_WARNING,
                      "Subtracting combined openings compound failed:", entity->entity);
      return false;
    }
  }
  return true;
  ...
  for (IfcSchema::IfcRelVoidsElement::list::it it = openings->begin(); it != openings->end(); ++it)
  {
    ...
    if (fes->is(IfcSchema::Type::IfcOpeningElement))
    {
      ...
      for (unsigned int i = 0; i < opening_shapes.size(); ++i)
      {
        TopoDS_Shape opening_shape_solid;
        const TopoDS_Shape &opening_shape_unlocated =
            ensure_fit_for_subtraction(opening_shapes[i].Shape(), opening_shape_solid);
...
bool IfcGeom::Kernel::convert_wire_to_face(const TopoDS_Wire &w, TopoDS_Face &face)
{
  ...
  {
    Logger::Error("Self-intersections with " + boost::lexical_cast<std::string>(results.Extent()) +
                  " cycles detected");
...
bool IfcGeom::Kernel::profile_helper(int numVerts, double *verts, int numFillets,
                                     int *filletIndices, double *filletRadii, gp_Trsf2d trsf,
  ...
  for (TopExp_Explorer exp1(wire, TopAbs_VERTEX); exp1.More(); exp1.Next())
  {
    ...
    for (TopExp_Explorer exp3(wire, TopAbs_EDGE); exp3.More(); exp3.Next())
    {
      ...
      if (edge_points.size() != 2)
        continue;
      if (edge_points[0].IsEqual(P1, getValue(GV_POINT_EQUALITY_TOLERANCE)))
        neighbors.push_back(edge_points[1]);
      else if (edge_points[1].IsEqual(P1, getValue(GV_POINT_EQUALITY_TOLERANCE)))
    ...
    for (TopExp_Explorer exp2(wire, TopAbs_VERTEX); exp2.More(); exp2.Next())
    {
      TopoDS_Vertex V2 = TopoDS::Vertex(exp2.Current());
      gp_Pnt P2 = BRep_Tool::Pnt(V2);
      if (P1.IsEqual(P2, getValue(GV_POINT_EQUALITY_TOLERANCE)))
        continue;
      bool found = false;
      for (std::vector<gp_Pnt>::const_iterator it = neighbors.begin(); it != neighbors.end(); ++it)
      {
        if ((*it).IsEqual(P2, getValue(GV_POINT_EQUALITY_TOLERANCE)))
        {
          found = true;
          break;
        }
    ...
    for (std::vector<gp_Pnt>::const_iterator it = non_neighbors.begin(); it != non_neighbors.end();
         ++it)
    {
  ...
  if (Logger::LOG_WARNING >= Logger::Verbosity()) {
          if (toler.Tolerance(s, 0) > t * 10.) {
                  ...
                  for (int i = 1; i <= shapes->Length(); ++i) {
                          ...
                          Logger::Warning("Tolerance of " +
  boost::lexical_cast<std::string>(toler.Tolerance(sub, 0)) + " on " + ss.str());
                  }
          }
  }
  */
...
TopoDS_Edge find_next(const TopTools_IndexedMapOfShape &edge_set,
...
bool IfcGeom::Kernel::flatten_shape_list(const IfcGeom::IfcRepresentationShapeItems &shapes,
                                         TopoDS_Shape &result, bool fuse)
{
  ...
  for (IfcGeom::IfcRepresentationShapeItems::const_iterator it = shapes.begin(); it != shapes.end();
       ++it)
  {
    TopoDS_Shape merged;
...
void IfcGeom::Kernel::remove_duplicate_points_from_loop(TColgp_SequenceOfPnt &polygon, bool closed,
                                                        double tol)
{
  if (tol <= 0.)
    tol = getValue(GV_PRECISION);
  tol *= tol;
...
void IfcGeom::Kernel::remove_collinear_points_from_loop(TColgp_SequenceOfPnt &polygon, bool closed,
                                                        double tol)
{
  if (tol <= 0.)
    tol = getValue(GV_PRECISION);
...
void IfcGeom::Kernel::sequence_of_point_to_wire(const TColgp_SequenceOfPnt &p, TopoDS_Wire &w,
                                                bool close)
{
...
IfcSchema::IfcRelVoidsElement::list::ptr
IfcGeom::Kernel::find_openings(IfcSchema::IfcProduct *product)
{
  ...
  for (;;)
  {
    ...
    if (rel_obdef->is(IfcSchema::Type::IfcElement) &&
        !rel_obdef->is(IfcSchema::Type::IfcOpeningElement))
    {
...
const IfcSchema::IfcMaterial *
IfcGeom::Kernel::get_single_material_association(const IfcSchema::IfcProduct *product)
{
  ...
  if (associated_materials->size() == 1)
  {
    ...
    if (!single_material && associated_material->as<IfcSchema::IfcMaterialLayerSetUsage>())
    {
      IfcSchema::IfcMaterialLayerSet *layerset =
          associated_material->as<IfcSchema::IfcMaterialLayerSetUsage>()->ForLayerSet();
...
IfcGeom::BRepElement<P> *IfcGeom::Kernel::create_brep_for_representation_and_product(
    const IteratorSettings &settings, IfcSchema::IfcRepresentation *representation,
    IfcSchema::IfcProduct *product)
{
    ...
    if (flatten_shape_list(shapes, merge, false))
    {
      if (count(merge, TopAbs_FACE) > 0)
      {
        ...
        if (convert_layerset(product, layers, styles, thickness))
        {
          ...
          if (product->as<IfcSchema::IfcWall>() &&
  ...
  if (single_material)
  {
    ...
    for (IfcGeom::IfcRepresentationShapeItems::iterator it = shapes.begin(); it != shapes.end();
         ++it)
    {
      if (!it->hasStyle() && s)
      {
  ...
  if (!settings.get(IfcGeom::IteratorSettings::DISABLE_OPENING_SUBTRACTIONS) && openings &&
      openings->size())
  {
    representation_id_builder << "-openings";
    for (IfcSchema::IfcRelVoidsElement::list::it it = openings->begin(); it != openings->end();
         ++it)
    {
    ...
    catch (const std::exception &e)
    {
      Logger::Message(Logger::LOG_ERROR,
                      std::string("Error processing openings for: ") + e.what() + ":",
                      product->entity);
      caught_error = true;
    }
    catch (...)
    ...
    {
      for (IfcGeom::IfcRepresentationShapeItems::iterator it = opened_shapes.begin();
           it != opened_shapes.end(); ++it)
      {
        it->prepend(trsf);
      }
      trsf = gp_Trsf();
    ...
    shape = new IfcGeom::Representation::BRep(element_settings, representation_id_builder.str(),
                                              opened_shapes);
  }
  ...
  return new BRepElement<P>(product->entity->id(), parent_id, name, product_type, guid,
                            context_string, trsf,
                            boost::shared_ptr<IfcGeom::Representation::BRep>(shape), product);
}

IfcSchema::IfcRepresentation *
IfcGeom::Kernel::representation_mapped_to(const IfcSchema::IfcRepresentation *representation)
{
...
IfcGeom::Kernel::products_represented_by(const IfcSchema::IfcRepresentation *representation)
{
  ...
  for (IfcSchema::IfcProductRepresentation::list::it it = prodreps->begin(); it != prodreps->end();
       ++it)
  {
    // http://buildingsmart-tech.org/ifc/IFC2x3/TC1/html/ifcrepresentationresource/lexical/ifcproductrepresentation.htm
    // IFC2x Edition 3 NOTE  Users should not instantiate the entity IfcProductRepresentation from
    // IFC2x Edition 3 onwards. It will be changed into an ABSTRACT supertype in future releases of
    // IFC.

    // IfcProductRepresentation also lacks the INVERSE relation to IfcProduct
    // Let's find the IfcProducts that reference the IfcProductRepresentation anyway
    products->push(
        (*it)->entity->getInverse(IfcSchema::Type::IfcProduct, -1)->as<IfcSchema::IfcProduct>());
  }
    ...
    if (is_identity_transform(map->MappingOrigin()))
    {
      ...
      for (IfcSchema::IfcMappedItem::list::it it = items->begin(); it != items->end(); ++it)
      {
        ...
        IfcSchema::IfcRepresentation::list::ptr reps =
            item->entity->getInverse(IfcSchema::Type::IfcRepresentation, -1)
        ...
        for (IfcSchema::IfcRepresentation::list::it jt = reps->begin(); jt != reps->end(); ++jt)
        {
          ...
          for (IfcSchema::IfcProductRepresentation::list::it kt = prodreps_mapped->begin();
               kt != prodreps_mapped->end(); ++kt)
          {
            IfcSchema::IfcProduct::list::ptr ps =
                (*kt)
                    ->entity->getInverse(IfcSchema::Type::IfcProduct, -1)
...
IfcGeom::BRepElement<P> *IfcGeom::Kernel::create_brep_for_processed_representation(
    const IteratorSettings & /*settings*/, IfcSchema::IfcRepresentation *representation,
    IfcSchema::IfcProduct *product, IfcGeom::BRepElement<P> *brep)
{
  int parent_id = -1;
  try
  {
  ...
  return new BRepElement<P>(product->entity->id(), parent_id, name, product_type, guid,
                            context_string, trsf, brep->geometry_pointer(), product);
}

IfcSchema::IfcObjectDefinition *
IfcGeom::Kernel::get_decomposing_entity(IfcSchema::IfcProduct *product)
{
...
std::map<std::string, IfcSchema::IfcPresentationLayerAssignment *>
IfcGeom::Kernel::get_layers(IfcSchema::IfcProduct *prod)
{
  ...
  if (prod->hasRepresentation())
  {
    ...
    for (IfcRepresentation::list::it it = representations->begin(); it != representations->end();
         ++it)
    {
...
template IFC_GEOM_API IfcGeom::BRepElement<float> *
IfcGeom::Kernel::create_brep_for_representation_and_product<float>(
    const IteratorSettings &settings, IfcSchema::IfcRepresentation *representation,
    IfcSchema::IfcProduct *product);

template IFC_GEOM_API IfcGeom::BRepElement<double> *
IfcGeom::Kernel::create_brep_for_representation_and_product<double>(
    const IteratorSettings &settings, IfcSchema::IfcRepresentation *representation,
    IfcSchema::IfcProduct *product);

template IFC_GEOM_API IfcGeom::BRepElement<float> *
IfcGeom::Kernel::create_brep_for_processed_representation<float>(
    const IteratorSettings &settings, IfcSchema::IfcRepresentation *representation,
    IfcSchema::IfcProduct *product, IfcGeom::BRepElement<float> *brep);

template IFC_GEOM_API IfcGeom::BRepElement<double> *
IfcGeom::Kernel::create_brep_for_processed_representation<double>(
    const IteratorSettings &settings, IfcSchema::IfcRepresentation *representation,
    IfcSchema::IfcProduct *product, IfcGeom::BRepElement<double> *brep);

std::pair<std::string, double>
IfcGeom::Kernel::initializeUnits(IfcSchema::IfcUnitAssignment *unit_assignment)
{
              ...
              named_unit->UnitType() == IfcSchema::IfcUnitEnum::IfcUnit_PLANEANGLEUNIT)
          {
            ...
            if (current_unit_magnitude != 0.)
            {
              ...
              else if (named_unit->is(IfcSchema::Type::IfcSIUnit))
              {
                ...
                {
                  current_unit_name =
                      IfcSchema::IfcSIPrefix::ToString(si_unit->Prefix()) + unit_name;
                }
...
bool IfcGeom::Kernel::convert_layerset(const IfcSchema::IfcProduct *product,
                                       ...
                                       std::vector<double> &thicknesses)
{
  ...
  IfcSchema::IfcRelAssociates::list::ptr associations = product->HasAssociations();
  for (IfcSchema::IfcRelAssociates::list::it it = associations->begin(); it != associations->end();
       ++it)
  {
    IfcSchema::IfcRelAssociatesMaterial *associates_material =
        (**it).as<IfcSchema::IfcRelAssociatesMaterial>();
  ...
  if (product->is(IfcSchema::Type::IfcWall))
  {
    ...
    {
      Logger::Message(Logger::LOG_WARNING,
                      "No edge found in axis representation:", product->entity);
      return false;
    }
    ...
    { /**< @todo Why always true? */
      if (axis_curve->DynamicType() == STANDARD_TYPE(Geom_Line))
      {
        ...
        reference_surface =
            new Geom_Plane(axis_line->Lin().Location(), axis_line->Lin().Direction() ^ gp::DZ());
      }
      ...
      {
        Logger::Message(Logger::LOG_ERROR,
                        "Unsupported underlying curve of Axis representation:", product->entity);
        return false;
      }
    }
    else
  ...
  else
  {
    IfcSchema::IfcExtrudedAreaSolid::list::ptr extrusions =
        body_representation->entity->file->traverse(body_representation)
            ->as<IfcSchema::IfcExtrudedAreaSolid>();

    if (extrusions->size() != 1)
    {
      Logger::Message(Logger::LOG_WARNING,
                      "No single extrusion found in body representation for:", product->entity);
      return false;
    }
    ...
    if (has_position)
    {
      ...
      {
        Logger::Message(Logger::LOG_ERROR,
                        "Failed to convert placement for extrusion of:", product->entity);
        return false;
      }
    }
    ...
    {
      Logger::Message(Logger::LOG_ERROR,
                      "Failed to convert direction for extrusion of:", product->entity);
      return false;
    }
  ...
  const bool positive =
      usage->DirectionSense() == IfcSchema::IfcDirectionSenseEnum::IfcDirectionSense_POSITIVE;
...
const Handle_Geom_Curve IfcGeom::Kernel::intersect(const Handle_Geom_Surface &a,
...
const Handle_Geom_Curve IfcGeom::Kernel::intersect(const TopoDS_Face &a,
                                                   const Handle_Geom_Surface &b)
{
...
bool IfcGeom::Kernel::intersect(const Handle_Geom_Curve &a, const TopoDS_Shape &b,
...
bool IfcGeom::Kernel::intersect(const Handle_Geom_Surface &a, const TopoDS_Shape &b,
...
bool IfcGeom::Kernel::project(const Handle_Geom_Curve &crv, const gp_Pnt &pt, gp_Pnt &p, double &u,
                              double &d)
{
...
bool IfcGeom::Kernel::find_wall_end_points(const IfcSchema::IfcWall *wall, gp_Pnt &start,
                                           gp_Pnt &end)
{
...
bool IfcGeom::Kernel::fold_layers(const IfcSchema::IfcWall *wall,
                                  const IfcRepresentationShapeItems &items,
                                  const std::vector<Handle_Geom_Surface> &surfaces,
                                  const std::vector<double> &thicknesses,
                                  std::vector<std::vector<Handle_Geom_Surface>> &result)
{
  bool folds_made = false;

  IfcSchema::IfcRelConnectsPathElements::list::ptr connections(
      new IfcSchema::IfcRelConnectsPathElements::list);
  ...
  typedef std::vector<std::pair<std::pair<IfcSchema::IfcConnectionTypeEnum::IfcConnectionTypeEnum,
                                          IfcSchema::IfcConnectionTypeEnum::IfcConnectionTypeEnum>,
                                const IfcSchema::IfcProduct *>>
  ...
  for (IfcSchema::IfcRelConnectsPathElements::list::it it = connections->begin();
       it != connections->end(); ++it)
  {
    IfcSchema::IfcRelConnectsPathElements *connection = *it;
    IfcSchema::IfcConnectionTypeEnum::IfcConnectionTypeEnum own_type =
        connection->RelatedElement() == wall ? connection->RelatedConnectionType()
                                             : connection->RelatingConnectionType();
    IfcSchema::IfcConnectionTypeEnum::IfcConnectionTypeEnum other_type =
        connection->RelatedElement() == wall ? connection->RelatingConnectionType()
  ...
  for (endpoint_connections_t::const_iterator it = endpoint_connections.begin();
       it != endpoint_connections.end(); ++it)
  {
  ...
  for (int idx = 0; idx < 2; ++idx)
  {
    ...
    */
    ...
    /*
    Additionally one could check whether the end points are of the wall are really ~1 LayerThickness
    away from each other for (endpoint_connections_t::const_iterator it =
    endpoint_connections.begin(); it != endpoint_connections.end(); ++it) {
            IfcSchema::IfcConnectionTypeEnum::IfcConnectionTypeEnum relating_connection_type =
    it->first.first; IfcSchema::IfcConnectionTypeEnum::IfcConnectionTypeEnum related_connection_type
    = it->first.second;
            ...
            find_wall_end_points(it->second->as<IfcSchema::IfcWall>(), other_axis_start,
    other_axis_end);
            ...
            const gp_Pnt& a = relating_connection_type ==
    IfcSchema::IfcConnectionTypeEnum::IfcConnectionType_ATSTART ? own_axis_start : own_axis_end;

            const gp_Pnt& b = related_connection_type ==
    IfcSchema::IfcConnectionTypeEnum::IfcConnectionType_ATSTART ? other_axis_start : other_axis_end;
       ...
       it != endpoint_connections.end(); ++it)
  {
    ...
    // If more than one wall connects to this start/end -point assume layers do not need to be
    // folded
    const int idx = connection_type == IfcSchema::IfcConnectionTypeEnum::IfcConnectionType_ATSTART;
    if (connection_type_count[idx] > 1)
      continue;

    const gp_Pnt &own_end_point =
        connection_type == IfcSchema::IfcConnectionTypeEnum::IfcConnectionType_ATEND
            ? own_axis_end
            : own_axis_start;
    ...
    for (surfaces_t::const_iterator jt = surfaces.begin() + 1; jt != surfaces.end() - 1;
         ++jt, ++result_vector)
    {
      ...
      if (!found_intersection && point_outside_param_range)
      {
        ...
        */
        ...
        for (curves_on_surfaces_t::const_iterator kt = layer_ends.begin(); kt != layer_ends.end();
             ++kt)
        {
          gp_Pnt p;
          gp_Vec v;
        ...
        if (intersection2.IsDone() && intersection2.NbPoints() == 1)
        {
          ...
          if (intersection3.IsDone() && intersection3.NbLines() == 1)
          {
            ...
            ShapeAnalysis_Curve sac;
            gp_Pnt layer_end_point_projected;
            double layer_end_point_param;
            sac.Project(layer_line, layer_end_point, 1e-3, layer_end_point_projected,
                        layer_end_point_param, false);
            ...
            if (dst.IsDone())
            {
              ...
              if (intersection4.IsDone() && intersection4.NbLines() == 1)
              {
                ...
                gp_Pnt layer_fold_point_projected;
                double layer_fold_point_param;
                sac2.Project(body_trim_curve, layer_fold_point, 1.e-7, layer_fold_point_projected,
                             layer_fold_point_param, false);
                Handle_Geom_Curve fold_curve = new Geom_OffsetCurve(
                    body_trim_curve->Reversed(),
                    layer_fold_point_projected.Distance(layer_fold_point), gp::DZ());

                Handle_Geom_Surface fold_surface =
                    new Geom_SurfaceOfLinearExtrusion(fold_curve, gp::DZ());
...
bool IfcGeom::Kernel::apply_folded_layerset(
    const IfcRepresentationShapeItems &items,
    const std::vector<std::vector<Handle_Geom_Surface>> &surfaces,
    const std::vector<const SurfaceStyle *> &styles, IfcRepresentationShapeItems &result)
{
  Bnd_Box bb;
  TopoDS_Shape input;
  ...
  else if (shells.size() == 1)
  {
    ...
    for (IfcRepresentationShapeItems::const_iterator it = items.begin(); it != items.end(); ++it)
    {
      ...
      {
        result.push_back(
            IfcRepresentationShapeItem(it->Placement(), b, styles[0] ? styles[0] : &it->Style()));
        result.push_back(
            IfcRepresentationShapeItem(it->Placement(), a, styles[1] ? styles[1] : &it->Style()));
      }
      else
      {
        continue;
  ...
  else
  {
    ...
    for (; it1 != items.end(); ++it1, ++it2)
    {
      ...
      {
        result.push_back(
            IfcRepresentationShapeItem(it1->Placement(), *it3, (*it4) ? (*it4) : &it1->Style()));
      }
    }
...
bool IfcGeom::Kernel::apply_layerset(const IfcRepresentationShapeItems &items,
                                     const std::vector<Handle_Geom_Surface> &surfaces,
                                     const std::vector<const SurfaceStyle *> &styles,
                                     IfcRepresentationShapeItems &result)
{
  if (surfaces.size() < 3)
  {
...
IfcSchema::IfcRepresentation *
IfcGeom::Kernel::find_representation(const IfcSchema::IfcProduct *product,
...
bool IfcGeom::Kernel::split_solid_by_surface(const TopoDS_Shape &input,
                                             const Handle_Geom_Surface &surface,
                                             TopoDS_Shape &front, TopoDS_Shape &back)
{
...
bool IfcGeom::Kernel::split_solid_by_shell(const TopoDS_Shape &input, const TopoDS_Shape &shell,
                                           TopoDS_Shape &front, TopoDS_Shape &back)
{
...
bool IfcGeom::Kernel::project(const Handle_Geom_Surface &srf, const TopoDS_Shape &shp, double &u1,
                              double &v1, double &u2, double &v2, double widen)
{
...
IfcGeom::Kernel::find_item_carrying_style(const IfcSchema::IfcRepresentationItem *item)
{
  ...
  while (item->is(IfcSchema::Type::IfcBooleanClippingResult))
  {
    ...
    item =
        (IfcSchema::IfcGeometricRepresentationItem *)((IfcSchema::IfcBooleanClippingResult *)item)
            ->FirstOperand();
    if (item->StyledByItem()->size())
    {
      return item;
...
bool IfcGeom::Kernel::is_identity_transform(IfcUtil::IfcBaseClass *l)
{
  ...
  else
  {
    throw IfcParse::IfcException(
        "Invalid valuation for IfcAxis2Placement / IfcCartesianTransformationOperator");
  }
}
...
bool IfcGeom::Kernel::wire_intersections(const TopoDS_Wire &wire, TopTools_ListOfShape &wires)
{
  ...
  for (int i = 2; i < n; ++i)
  {
    ...
    for (std::vector<int>::const_iterator it = js.begin(); it != js.end(); ++it)
    {
      ...
      GeomAPI_ExtremaCurveCurve ecc(BRep_Tool::Curve(wd->Edge(i + 1), u11, u12),
...
bool IfcGeom::Kernel::fit_halfspace(const TopoDS_Shape &a, const TopoDS_Shape &b, TopoDS_Shape &box,
                                    double &height)
{
  TopExp_Explorer exp(b, TopAbs_FACE);
  if (!exp.More())
...
#if OCC_VERSION_HEX < 0x60900
bool IfcGeom::Kernel::boolean_operation(const TopoDS_Shape &a, const TopTools_ListOfShape &b,
                                        BOPAlgo_Operation op, TopoDS_Shape &result)
{
  result = a;
  TopTools_ListIteratorOfListOfShape it(b);
  for (; it.More(); it.Next())
...
bool IfcGeom::Kernel::boolean_operation(const TopoDS_Shape &a, const TopoDS_Shape &b,
                                        BOPAlgo_Operation op, TopoDS_Shape &result)
{
  bool succesful = true;
...
#else
bool IfcGeom::Kernel::boolean_operation(const TopoDS_Shape &a, const TopTools_ListOfShape &b_,
                                        BOPAlgo_Operation op, TopoDS_Shape &result,
                                        double fuzziness)
{
  bool success = false;
  ...
  double min_len =
      (std::min)(min_edge_length(a), min_vertex_edge_distance(a, getValue(GV_PRECISION)));
  TopTools_ListIteratorOfListOfShape it(b);
  for (; it.More(); it.Next())
  {
  ...
  if (builder->IsDone())
  {
    ...
    if (success)
    {
      ...
      if (success)
      {
        ...
        double min_len_check =
            (std::min)(min_edge_length(r), min_vertex_edge_distance(r, getValue(GV_PRECISION)));
